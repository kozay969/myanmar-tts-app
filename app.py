...
def parse_style_prompt(prompt, speed_multiplier=1.0, echo_level=0):
    ...
    if 'ပဲ့တင်အနည်းဆုံး' in prompt:
        echo_value = max(echo_value, 5)
        effects.append("ပဲ့တင်အနည်းဆုံး")
    elif 'ပဲ့တင်နည်းနည်း' in prompt:
        echo_value = max(echo_value, 10)
        effects.append("ပဲ့တင်နည်းနည်း")
    elif 'ပဲ့တင်များများ' in prompt or 'ဂူထဲက' in prompt:
        echo_value = max(echo_value, 50)
        effects.append("ပဲ့တင်များများ")
    elif any(w in prompt for w in ['ပဲ့တင်', 'echo']):
        echo_value = max(echo_value, 30)
...
if st.button("🚀 ထုတ်မယ်", use_container_width=True, type="primary"):
    if text.strip():
        with st.spinner("အသံထုတ်နေတယ်... ခဏစောင့်"):
            # BGM ရှိရင် temp file မှာ save မယ်
            bgm_path = None
            if bgm_file:
                with tempfile.NamedTemporaryFile(delete=False, suffix=".mp3") as tmp:
